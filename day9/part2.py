...
Part 2: It's the same but instead of the sum of the differences of the last digit, it's the difference or sum (depending on the row) of the first digits in each row of differences.
"""
    ...
    with open(args.input_file, "r") as file:
        for line in file:
            ...
            while not found:
                ...
                # check if the row is even and add the first digit, otherwise subtract it
                if idx % 2 == 0:
                ...
                if all(x == differences[0] for x in differences):
                    found = True
                    final_sum += (first_digits - difference_of_differences)
